        ...
        it('should create event filter', function (done) {
            ...
            provider.injectValidation(function (payload) {
                if (step === 0) {
                    ...
                    assert.equal(payload.method, 'owt_newFilter');
                    ...
                    assert.equal(payload.jsonrpc, '2.0');
                    assert.equal(payload.method, 'owt_getFilterLogs');
                    ...
                    var r = payload.filter(function (p) {
                        return p.jsonrpc === '2.0' && p.method === 'owt_getFilterChanges' && p.params[0] === '0x3';
                    });
                    assert.equal(r.length > 0, true);
                }
            });

            var contract = web3.owt.contract(desc).at(address);
