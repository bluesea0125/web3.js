...
describe('web3.owt.contract', function() {
    it('should create simple contract with one method from abi with explicit type name', function () {
        ...
        // when
        var web3 = new Web3();
        var myCon = web3.owt.contract(description).at(address);
    ...
    it('should create contract with nondefault constructor', function (done) {
        ...
        provider.injectValidation(function (payload) {
            if (steps === 1) {
                ...
                assert.equal(payload.jsonrpc, '2.0');
                assert.equal(payload.method, 'owt_newBlockFilter');
                steps++;
            }
        });

        web3.owt.contract(description).new(2, {from: address, data: code}, function(e, myCon){
            myCon._owt._requestManager.reset();
            done();
        });
    });
});
